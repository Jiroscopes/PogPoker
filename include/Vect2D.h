...
struct Vect2D
{
    ...
    // addition assignment
    inline friend Vect2D operator+=(Vect2D<T>& v1, const Vect2D<T>& v2)
    {
        v1.x += v2.x;
        v1.y += v2.y;
        return v1;
    ...
    // division
    inline Vect2D<T> operator/(const T divisor) const
    {
        return (divisor != 0 ? Vect2D<T>(x / divisor, y / divisor) : Vect2D<T>());
    }
    ...
    inline friend std::ostream& operator<<(std::ostream& stream, const Vect2D& v)
    {
        stream << "(" << v.x << "," << v.y << ")";
        return stream;
    }
